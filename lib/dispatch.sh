...
_interactive_reason() {
  ...
  printf '%s' $'You are entering the **'"$stage_name"$'** stage.\n\n**Run this stage YOURSELF, in THIS conversation, as a normal back-and-forth chat with the user.** Follow the methodology below: one question at a time, lead with your recommendation and rationale, offer options. Write each question as plain conversational text, then **end your turn** — control returns cleanly to the user and their reply arrives as the next message. (You do NOT need AskUserQuestion to "pause" — ending your turn already yields to the user; the orchestrator stays silent during the conversation. Do NOT use AskUserQuestion, do NOT spawn a subagent, do NOT call the Agent tool, and do NOT use SendMessage — delegating this stage is blocked and only wastes a turn.)\n\n'"$output_block""$skill_body"$'\n\n---\n\nWhen this stage is done (artifact written and user-approved), call:\n\n```\nmcp__atelier__atelier_signal({type:"stage_complete", pipelineId:"'"$pid"$'", verdict:"...", outputPath:"..."})\n```\n\nThen end your turn.'
}
